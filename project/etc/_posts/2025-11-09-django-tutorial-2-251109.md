---
layout: post
title: "[django] Django Tutorial - TEST CRUD API 2"
date: 2025-11-09 23:32:20 +0900
description: A lightweight Django project demonstrating basic CRUD operations and MVC structure.
img: # Add image post (optional)
fig-caption: # Add figcaption (optional)
categories: [project, etc]
---
# Django REST Framework를 사용하여 게시판 CRUD API
## View setting
### `board/views.py` (ViewSet)
```python
class PostViewSet(viewsets.ModelViewSet):
    queryset = Post.objects.all()
    serializer_class = PostSerializer
```
- `viewsets.ModelViewSet` → CRUD 기능 전체 제공
  - Spring Boot에서 `@RestController + Repository` + CRUD 메서드 모음 느낌
- `queryset` → 조회할 대상 DB
- `serializer_class` → 데이터 직렬화/역직렬화 (JSON ↔ DB)

#### (1) `get_serializer_class`
- 목록 조회(`list`)만 가볍게 **PostListSerializer** 사용
- 상세 조회, 생성, 수정 등은 PostSerializer 사용
- Spring Boot에서 **목록용 DTO / 상세용 DTO**를 선택하는 것과 유사

#### (2) CRUD 메서드 오버라이드
- `list`, `create`, `retrieve`, `update`, `partial_update`, `destroy`
- 역할: HTTP 요청 처리 → 모델 + serializer → Response 반환

## URL Routing
### `board/urls.py` (앱 단위 URL)
```python
router = DefaultRouter()
router.register('board', PostViewSet, basename='post')

urlpatterns = [
    path('v1/', include(router.urls)),
]
```
### `crud_project/urls.py` (프로젝트 단위 URL)
```python
urlpatterns = [
path('admin/', admin.site.urls),
path('', include('board.urls')),
]
```
- Django 프로젝트 루트에서 앱 URL 포함
- `/admin/` → Django Admin
- `/v1/board/...` → board 앱에서 라우팅
- Spring Boot에서는 프로젝트 전역 `@RequestMapping`과 비슷

## 🔹 전체 흐름 요약
1. 모델(Post) → DB 구조 정의
2. Serializer(PostSerializer, PostListSerializer) → JSON 변환
3. ViewSet(PostViewSet) → HTTP 요청 처리
4. Router(board/urls.py) → URL 자동 매핑
5. 루트 urls.py → 앱 URL 포함
→ `/v1/board/...`에서 바로 CRUD 테스트 가능 ✅

## API TEST
### 게시글 생성 (POST)
- `http://127.0.0.1:8000/v1/board/`
```bash
curl -X POST http://localhost:8000/v1/board/ \
  -H "Content-Type: application/json" \
  -d '{
    "title": "첫 번째 게시글",
    "content": "게시글 내용입니다.",
    "author": "홍길동"
  }'
```
![img.png](/assets/img/posts/project/etc/django/3/img1.png)
![img.png](/assets/img/posts/project/etc/django/3/img2.png)
### 게시글 목록 조회 (GET)
```bash
curl http://localhost:8000/api/posts/
```
![img.png](/assets/img/posts/project/etc/django/3/img3.png)
### 게시글 상세 조회 (GET)
```bash
curl http://localhost:8000/api/posts/1/
```
![img.png](/assets/img/posts/project/etc/django/3/img4.png)
### 게시글 수정 (PATCH)
```bash
curl -X PATCH http://localhost:8000/api/posts/1/ \
  -H "Content-Type: application/json" \
  -d '{
    "title": "수정된 제목"
  }'
```
![img.png](/assets/img/posts/project/etc/django/3/img5.png)
### 게시글 삭제 (DELETE)
```bash
curl -X DELETE http://localhost:8000/api/posts/1/
```
![img.png](/assets/img/posts/project/etc/django/3/img6.png)
![img_1.png](/assets/img/posts/project/etc/django/3/img7.png)
